...
- **Language selector / i18n (added 2026-07-15, extended 2026-07-16)**: `LANGUAGES`/`TRANSLATIONS`/`t()`/`getLang()`/`setLang()` in `index.html`. Now 5 languages — English, Spanish, French, Portuguese, Arabic. Deliberately bounded scope — only nav labels, the staff and client portal auth screens (sign-in/up, forgot password, set new password), and the client portal dashboard are translated. The rest of the staff CRM (client records, invoices, leads, campaigns, reports, AI assistant, platform admin) is still English-only — translating the whole app is a much bigger job (hundreds of strings across ~12 pages) than this pass covered; the infrastructure supports it, extending coverage is additive. Language is a `localStorage` preference (per-browser, not per-clinic) since the auth screens render before any clinic is loaded. To extend: add a language to `LANGUAGES`+`TRANSLATIONS`, or add more translated strings by adding a key to every language block and calling `t('your.key')` at the render site.
