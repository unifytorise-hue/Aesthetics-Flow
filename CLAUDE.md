...
- **Language selector / i18n (added 2026-07-15)**: `LANGUAGES`/`TRANSLATIONS`/`t()`/`getLang()`/`setLang()` in `index.html`. Deliberately bounded scope — only nav labels, the staff and client portal auth screens (sign-in/up, forgot password, set new password), and the client portal dashboard are translated (English/Spanish/French). The rest of the staff CRM (client records, invoices, leads, campaigns, reports, AI assistant, platform admin) is still English-only. Language is a `localStorage` preference (per-browser, not per-clinic) since the auth screens render before any clinic is loaded. To extend: add a language to `LANGUAGES`+`TRANSLATIONS`, or add more translated strings by adding a key to every language block and calling `t('your.key')` at the render site.
- **Currency selector (added 2026-07-15)**: the free-text currency field in clinic Settings is now a dropdown of ~140 world currencies (`CURRENCIES`). Still stores just the symbol in `clinic_settings.currency` (unchanged column/type) — `fmtMoney()` wasn't touched, so this is a UI improvement only, not real per-currency number formatting. Because several currencies share a symbol (e.g. `$` for USD/MXN/ARS/...), the dropdown's pre-selected option after reload may not be the exact currency originally picked, though the displayed symbol is always correct.
